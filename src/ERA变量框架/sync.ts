...
export const resyncStateOnHistoryChange = async () => {
  ...
  //
  // **原因**:
  // 为了处理一种 `resync` 逻辑无法检测到的特殊情况：在最后一楼删除 swipe。
  // 此时，后一个 swipe 的内容会“顶替”上来，但消息变量（包括其MK）却保持不变。
  // 这导致 `resync` 的 MK 序列比对无法发现任何差异，从而错误地跳过修复。
  //
  // 此处的逻辑直接复刻了旧 `calibrate` 函数的核心思想：无条件地回滚上一个状态，
  // 然后根据当前最后一楼的**实际内容**进行重写，从而强制修复“内容-变量错位”问题。
  // ==================================================================
